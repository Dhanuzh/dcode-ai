...
Session state and event streams are stored as JSON and JSONL under `<workspace>/.dcode-ai/sessions/` (see `runtime::session_store`). Global user config lives under `~/.dcode-ai/config.toml`. No separate SQLite control-plane database is used in this workspace.
