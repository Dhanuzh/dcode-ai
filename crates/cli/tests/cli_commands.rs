...
fn write_local_config_contents(workspace: &Path, contents: &str) {
    let config_dir = workspace.join(".dcode-ai");
...
/// Same selection rule as `dcode-ai --resume` (latest `updated_at`).
#[tokio::test]
async fn resume_targets_session_with_latest_updated_at() {
    ...
    let store = dcode_ai_runtime::session_store::SessionStore::new(
        temp.path().join(".dcode-ai").join("sessions"),
    );
...
#[test]
fn index_build_writes_cli_index_json_under_dcode_ai_home() {
    ...
    let workspaces = home.path().join(".dcode-ai/workspaces");
...
fn index_show_and_build_json_status() {
    ...
    let out = Command::cargo_bin("dcode-ai")
        .expect("binary")
        .current_dir(ws.path())
        .env("HOME", home.path())
