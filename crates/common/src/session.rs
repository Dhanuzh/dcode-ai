...
pub struct SessionMeta {
    ...
    /// Branch name the session operates on (e.g. `dcode-ai/<session-id>`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl OrchestrationContext {
    pub fn from_env() -> Option<Self> {
        ...
        for (key, value) in env::vars() {
            if let Some(meta_key) = key.strip_prefix("DCODE_AI_ORCH_META_")
                && !value.trim().is_empty()
            {
        ...
        let ctx = Self {
            orchestrator: non_empty_env("DCODE_AI_ORCH_NAME"),
            run_id: non_empty_env("DCODE_AI_ORCH_RUN_ID"),
            task_id: non_empty_env("DCODE_AI_ORCH_TASK_ID"),
            task_ref: non_empty_env("DCODE_AI_ORCH_TASK_REF"),
            parent_run_id: non_empty_env("DCODE_AI_ORCH_PARENT_RUN_ID"),
            callback_url: non_empty_env("DCODE_AI_ORCH_CALLBACK_URL"),
            metadata,
        };
...
mod tests {
    ...
    fn orchestration_context_reads_env_contract() {
        let vars = [
            ...
            ("DCODE_AI_ORCH_TASK_REF", "issue/99"),
            ("DCODE_AI_ORCH_PARENT_RUN_ID", "run-122"),
            ("DCODE_AI_ORCH_CALLBACK_URL", "http://localhost/callback"),
            ("DCODE_AI_ORCH_META_CHANNEL", "ticket"),
        ];
