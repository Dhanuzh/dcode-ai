...
use dcode_ai_common::config::{DcodeAiConfig, OpenRouterConfig};
    ...
    pub fn from_config(config: &DcodeAiConfig) -> Result<Self, ProviderError> {
        ...
        let api_key = openrouter.resolve_api_key().ok_or_else(|| {
            ProviderError::Configuration(format!(
                "missing OpenRouter API key; set {} or provide `provider.openrouter.api_key` in config (or open dcode-ai and run /login or /connect)",
...
mod tests {
    ...
    async fn openrouter_provider_sends_optional_headers_and_streams_usage() {
            ...
            assert!(
                request
                    ...
                    .any(|header| header.field.equiv("http-referer")
                        && header.value.as_str() == "https://dcode-ai.test")
            );
            assert!(
                request
                    .headers()
        ...
        config.provider.openrouter.site_url = Some("https://dcode-ai.test".into());
